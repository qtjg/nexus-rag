...
import { askKnowledge, configureEnterpriseSso, configureIngestionRetrySchedule, createCollection, createConnectorConfiguration, createGitRepositorySnapshot, createServiceApiKey, decideReleaseApproval, deleteConnectorConfiguration, getGitReviewHistory, getOrganizationAnalytics, getWorkspace, ingestFileSource, ingestTextSource, inviteMember, listGitRepositorySnapshots, listServiceApiKeys, removeSource, replayIngestionJob, reviewGitRepositorySnapshot, revokeInvitation, revokeMember, revokeServiceApiKey, rotateServiceApiKey, setConnectorConfigurationState, submitFeedback, updateMemberAccess, updateOrganizationPolicy } from "./nexus/service";
...
export const appRouter = router({
  ...
  nexus: router({
    ...
    gitSnapshots: protectedProcedure.input(orgInput).query(({ ctx, input }) => listGitRepositorySnapshots({ ...input, userId: ctx.user.id })),
    registerGitSnapshot: protectedProcedure.input(orgInput.extend({ collectionId: z.number().int().positive(), repositoryLabel: z.string().trim().min(1).max(160), repositoryReference: z.string().trim().max(500).optional(), revision: z.string().trim().min(1).max(128), baseRevision: z.string().trim().max(128).optional(), kind: z.enum(["snapshot", "diff"]), content: z.string().trim().min(1).max(160_000) })).mutation(({ ctx, input }) => createGitRepositorySnapshot({ ...input, userId: ctx.user.id })),
    reviewGitSnapshot: protectedProcedure.input(orgInput.extend({ snapshotId: z.number().int().positive(), mode: z.enum(["deterministic", "ai_assisted"]).default("deterministic") })).mutation(({ ctx, input }) => reviewGitRepositorySnapshot({ ...input, userId: ctx.user.id })),
    gitReviewHistory: protectedProcedure.input(orgInput.extend({ snapshotId: z.number().int().positive() })).query(({ ctx, input }) => getGitReviewHistory({ ...input, userId: ctx.user.id })),
